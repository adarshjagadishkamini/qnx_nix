...
static char* find_store_path_for_boot_lib(const char* boot_path) {
    ...
    while ((entry = readdir(dir)) != NULL) {
        ...
        // Check if this store path ends with our library name
        if (strstr(entry->d_name, lib_name) != NULL) {
            // Construct full path with length checks
            char full_path[PATH_MAX];
            char store_lib[PATH_MAX];
            int ret;

            ret = snprintf(full_path, PATH_MAX, "%s/%s", NIX_STORE_PATH, entry->d_name);
            if (ret < 0 || ret >= PATH_MAX) {
                continue; // Path too long, skip this entry
            }
            
            ret = snprintf(store_lib, PATH_MAX, "%s/%s", full_path, lib_name);
            if (ret < 0 || ret >= PATH_MAX) {
                continue; // Path too long, skip this entry
            }
            
            struct stat st;
            if (stat(store_lib, &st) == 0 && S_ISREG(st.st_mode)) {
         ...
         if (stat(source_subdir_path, &st_subdir) == 0 && S_ISDIR(st_subdir.st_mode)) {
             ...
             while ((entry = readdir(dir)) != NULL) {
                 ...
                 if (strcmp(subdirs[i], "bin") == 0) {
                     // Create wrapper script for items in bin/
                     struct stat item_st;
                     if(stat(source_item_path, &item_st) == 0 && S_ISREG(item_st.st_mode)) {
                          if (create_wrapper_script(profile_item_path, source_item_path, store_path) != 0) {
                              fprintf(stderr, "    Failed to create wrapper for %s. Skipping.\n", entry->d_name);
                          }
                     } else {
                          printf("    Skipping non-regular file in bin: %s\n", entry->d_name);
                     }
                 } else {
