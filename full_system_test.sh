...
rm -rf /data/nix
rm nix-store nix-shell-qnx nix.conf
./nix-store --init

# Add boot libraries
echo "2. Adding boot libraries..."
./nix-store --add-boot-libs-bins >/dev/null
...
sleep 3
# Profile 2: tail only
./nix-store --install $TAIL_PATH test1
sleep 4
...
./nix-store --switch-generation test3 timestamp 
#ls -l /data/nix/profiles/test3/bin/
...
# Add boot libraries
echo "2. Adding boot libraries..."
#./nix-store --add-boot-libs-bins
# Create and set up a test profile
./nix-store --create-profile test1
ls -l /data/nix/profiles/test1/bin/
./nix-store --add-with-deps head head
HEAD_PATH=$(find /data/nix/store -name "*-head" -type d)
./nix-store --install $HEAD_PATH test1

# Enter shell and verify only profile binaries work
./nix-shell-qnx test1
which head  # Should show profile path
which ssh   # Should fail
ssh         # Should fail
exit

# Verify outside shell can access system commands
which ssh   # Should show system path


vi nix.conf
#edit allow_system_binaries = true

#Open shell to verify if we can access system binaries
./nix-shell-qnx test1
which ssh   # Should show system path
ssh         # Should work
exit
